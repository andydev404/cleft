...
static CARD_CANDIDATE_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\d[\d -]{11,22}\d").unwrap());
...
// Entropy is checked per whitespace-delimited token (an API key is one
// token), not over the whole clip — scanning full text would flag ordinary
// prose. Base64 blobs, hashes and UUIDs can still trip this; revisit once
// the content classifier can exempt known-safe types.
