...
// `Image` is left out of this enum — nothing in the capture pipeline reads
// image data yet (clipboard.rs only calls read_text()). Add it once image
// capture lands.
...
mod tests {
    ...
    fn detects_unix_path() {
        assert_eq!(
            classify("/Users/dev/project/main.rs"),
